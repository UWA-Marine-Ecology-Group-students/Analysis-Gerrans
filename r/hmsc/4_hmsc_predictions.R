...
ncovrast  <- crop(covrast, nsiteext)
plot(ncovrast)

# fix the coordinate system so it matches our sample crs
proj4string(ncovrast) <- sppcrs
ncovrast <- projectRaster(ncovrast, crs = wgscrs)
ncovdf   <- as.data.frame(ncovrast, xy = TRUE, na.rm = TRUE)
head(ncovdf)
saveRDS(ncovdf, "output/covariate_grid_north.rds")

# save this as the grid file so you dont have to run the code above

### prepare the grid and make predictions
ncovdf      <- readRDS("output/covariate_grid_north.rds")
...
xy_grid    <- as.matrix(cbind(ncovdf$y, ncovdf$x))
XData_grid <- ncovdf[ , 3:10]
summary(XData_grid)
...
for(i in 1:4000){
  predi  <- cbind(ncovdf[1:2], predictsp[[i]])                                   # join the coordinates and a single set of predictions
